  ...
  return (
    <div className="p-3 sm:p-6 md:p-8 max-w-6xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          {canManage && !activePlan && (
            <Button 
              ...
              className="bg-[#1e1b4b] hover:bg-[#2e2b5b] text-white rounded-md px-4 sm:px-6 h-9"
            >
      ...
      {isPending || !orgId ? (
        ...
        <div className="border border-dashed border-slate-200 rounded-xl bg-white p-6 sm:p-12 md:p-16 flex flex-col items-center justify-center text-center shadow-sm">
          <div className="bg-slate-50 p-4 rounded-2xl mb-4 border border-slate-100">
            <Calendar className="w-8 h-8 text-slate-400" />
          </div>
          <h3 className="text-base font-semibold text-slate-800 mb-1">
            No work plan for {selectedYear}
          </h3>
          <p className="text-sm text-slate-500 max-w-sm mb-6">
            Get started by importing a pre-existing CSV or creating a blank canvas to organize your agenda and priorities.
          </p>

          {canManage && (
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-6 w-full max-w-xs sm:max-w-none justify-center">
            <input 
              id="csv-upload-empty"
              type="file" 
              accept=".csv"
            ...
            <Label 
              ...
              className={`cursor-pointer h-10 px-5 border border-slate-200 rounded-lg bg-white hover:bg-slate-50 text-slate-700 flex items-center justify-center text-sm font-medium shadow-sm transition-colors ${isUploading ? 'opacity-50' : ''}`}
            >
              {isUploading ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <FileUp className="w-4 h-4 mr-2 text-slate-500" />}
              Import CSV
            </Label>
            ...
            <Button 
              ...
              className="bg-[#1e1b4b] hover:bg-[#2e2b5b] text-white rounded-lg h-10 px-5 shadow-sm flex items-center justify-center gap-2 text-sm font-medium"
            >
        ...
        <div className="border border-slate-200 rounded-xl bg-white shadow-sm overflow-hidden">
          <div className="p-4 sm:p-6 border-b bg-slate-50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
            ...
            {canManage && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Label 
                  ...
                  className={`cursor-pointer h-9 px-3 sm:px-4 border border-slate-200 rounded-md bg-white hover:bg-slate-50 text-slate-700 flex items-center text-sm font-medium transition-colors ${isUploading ? 'opacity-50' : ''}`}
                >
                  {isUploading ? <Loader2 className="w-4 h-4 mr-1.5 sm:mr-2 animate-spin" /> : <FileUp className="w-4 h-4 mr-1.5 sm:mr-2" />}
                  Import CSV
                </Label>

                <AlertDialog>
                  <AlertDialogTrigger render={
                    <Button variant="outline" className="h-9 text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200 px-3">
                      <Trash2 className="w-4 h-4 mr-1.5 sm:mr-2" />
                      <span className="hidden sm:inline">Reset Entire Plan</span>
                ...
                <Button onClick={() => setIsAddModalOpen(true)} className="bg-[#1e1b4b] hover:bg-[#2e2b5b] text-white h-9 px-3 sm:px-4">
                  + Add Item
                </Button>
              </div>
            )}
            ...
            {activePlan.items?.length > 0 ? (
              <table className="w-full text-sm text-left min-w-[500px] sm:min-w-[600px]">
                <thead className="bg-slate-50 text-slate-500 border-b">
                  <tr>
                    <th className="px-3 sm:px-6 py-3 font-medium">Month</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {activePlan.items.map((item: any) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-3 sm:px-6 py-3 sm:py-4 font-medium text-slate-700 whitespace-nowrap">
                        {new Date(selectedYear, item.month - 1).toLocaleString('default', { month: 'long' })}
                      </td>
                      <td className="px-3 sm:px-6 py-3 sm:py-4 text-slate-800 font-medium">
                        <div className="truncate max-w-[200px] sm:max-w-md">{item.title}</div>
                        {item.description && (
                          <div className="text-xs text-slate-500 mt-0.5 truncate max-w-[200px] sm:max-w-md">{item.description}</div>
                        )}
                      </td>
              ...
              <div className="p-8 sm:p-12 text-center text-slate-500 text-sm">
                No items added to this plan yet. Click "+ Add Item" or "Import CSV" above.
              </div>
            )}
          </div>
        </div>
